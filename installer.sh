...
surat=$(curl -sS https://raw.githubusercontent.com/vpn-script/permission/aio/ipmini | grep -w $ipes | awk '{print $4}')
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/vpn-script/bot_panel/aio/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
